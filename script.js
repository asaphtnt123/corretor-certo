...
function setupScrollNavigation() {
    document.querySelectorAll('.destaque-container').forEach(container => {
        const scrollContainer = container.querySelector('.highlight-scroll');
        if (!scrollContainer) return;
        
        // Cria botões de navegação
        const navDiv = document.createElement('div');
        navDiv.className = 'scroll-nav';
        
        const leftBtn = document.createElement('button');
        leftBtn.innerHTML = '<i class="fas fa-chevron-left"></i>';
        leftBtn.setAttribute('aria-label', 'Rolar para a esquerda');
        
        const rightBtn = document.createElement('button');
        rightBtn.innerHTML = '<i class="fas fa-chevron-right"></i>';
        rightBtn.setAttribute('aria-label', 'Rolar para a direita');
        ...
        const updateButtons = () => {
            leftBtn.disabled = scrollContainer.scrollLeft <= 10;
            rightBtn.disabled = scrollContainer.scrollLeft >= 
...
// Chamar após carregar os destaques
document.addEventListener('DOMContentLoaded', function() {
    // Observar mudanças no DOM para containers de destaque
    const observer = new MutationObserver(function(mutations) {
        if (document.querySelector('.destaque-container')) {
